...
class Cart extends Front_Controller {
	...
	function index($offset=false)
	{
		...
		$data['homepage']			= true;
                ...
                $data['last_review'] = $this->Product_model->get_last_review_products();
		
                foreach ($data['last_review'] as &$p)
		{
			$p->images	= (array)json_decode($p->images);
			$p->options	= $this->Option_model->get_product_options($p->id);
		}
                
                $product_count = $this->Product_model->get_last_create_products_count();
		...
		$data['last_create']= $this->Product_model->get_last_create_products($config['per_page'], $offset);
		foreach ($data['last_create'] as &$p)
		{
			$p->images	= (array)json_decode($p->images);
			$p->options	= $this->Option_model->get_product_options($p->id);
		}
	...
	function page($id = false)
	{
		...
		$data['gift_cards_enabled'] = $this->gift_cards_enabled;
                ...
                $data['page_children'] = $this->Page_model->get_pages($id);
                
                //var_dump($data['page_children']);die;
        ...
        function city($name)
	{
		...
		$product_count = $this->Product_model->count_products_by_city($name);
		...
		if($name == $base_url[count($base_url)])
		{
			$page	= 0;
			$segments++;
		}
		...
		$data['products']	= $this->Product_model->get_products_by_city($name, $config['per_page'], $page, $sort_by['by'], $sort_by['sort']);
	...
	function product($id)
	{
                $you_watch_this = false;
                $all_item = $this->session->userdata('cart_contents');
                if(isset($all_item['items']) && $all_item['items']){
                    foreach($all_item['items'] as $item){
                        if($item['id'] == $id){
                            $you_watch_this=true;
                            break;
                        }
                ...
                $data['you_watch_this'] = $you_watch_this;
		//get the product
		$data['product']	= $this->Product_model->get_product($id);
		$this->Product_model->change_review_time($id);
	...
	function add_to_cart()
	{
		...
		$post_options 	= $this->input->post('option');
                ...
                $se = $this->session->userdata('cart_contents');
                $watcher_id = $se['customer']['id'];
                
                $watch_data_insert = array(
                                        'watcher_id' => $watcher_id,
                                        'product_id' => $product_id,
                                        'date'       => date('Y-m-d H:i:s')
                                     );
                $this->Product_model->add_watch_product($watch_data_insert);
	...
	function giftcard()
	{
        ...
        function submit_your_content(){
            ...
            if($this->input->post('submitted')){
                ...
                $this->Product_model->add_your_сontent($data);
                $data['contact_message'] = 'Your data has been sent';
                $this->view('submit_your_content',$data);
            }else{
                $this->view('submit_your_content');
            }
        }
        
        function add_remove_request(){
            ...
            $this->Product_model->add_delete_request($data);
            
            echo 1;
        }
        
        public function update_ban_list(){
            $active_list = $this->Product_model->get_active_delete_request();
            foreach($active_list as $one_record){
                $product = $this->Product_model->get_product($one_record->product_id);
                 
                if(!$this->Product_model->check_list($product->escort_phone)){
                    
                    //var_dump($product->escort_phone);die;
                    $data = array(
                        'phone' => $product->escort_phone
                    );
                    //var_dump($one_record->phone);die;
                    $this->Product_model->add_banned_phone($data,$one_record->id);
                    
                    $this->Product_model->disabled_product_by_phone($product->escort_phone);
                }
            }
        }
}