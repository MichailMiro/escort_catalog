...
class Products extends Admin_Controller {	
	...
	function delete($id = false)
	{
        ...
        private function get_escort_products() {
            $this->db->dbprefix = "";
            $res = $this->db->get('adv');
            $this->db->dbprefix = "gc_";
            return $res->result();
        }
        ...
        $escort_products = $this->get_escort_products();
        

        if ($escort_products) {
            foreach ($escort_products as $escort_product) {
                
                    $escort_imgs = $escort_product->images;
                    $escort_imgs = explode(';',$escort_imgs);
                    
                    
                    if(!file_exists(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city)){
                        mkdir(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city);
                        chmod(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/small/'.$escort_product->city)){
                        mkdir(REAL_PATH.'/uploads/images/small/'.$escort_product->city);
                        chmod(REAL_PATH.'/uploads/images/small/'.$escort_product->city,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/medium/'.$escort_product->city)){
                        mkdir(REAL_PATH.'/uploads/images/medium/'.$escort_product->city);
                        chmod(REAL_PATH.'/uploads/images/medium/'.$escort_product->city,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/full/'.$escort_product->city)){
                        mkdir(REAL_PATH.'/uploads/images/full/'.$escort_product->city);
                        chmod(REAL_PATH.'/uploads/images/full/'.$escort_product->city,0777);
                    }
                    
                    
                    
                    if(!file_exists(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city.'/'.$escort_product->code)){
                        mkdir(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city.'/'.$escort_product->code);
                        chmod(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city.'/'.$escort_product->code,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/small/'.$escort_product->city.'/'.$escort_product->code)){
                        mkdir(REAL_PATH.'/uploads/images/small/'.$escort_product->city.'/'.$escort_product->code);
                        chmod(REAL_PATH.'/uploads/images/small/'.$escort_product->city.'/'.$escort_product->code,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/medium/'.$escort_product->city.'/'.$escort_product->code)){
                        mkdir(REAL_PATH.'/uploads/images/medium/'.$escort_product->city.'/'.$escort_product->code);
                        chmod(REAL_PATH.'/uploads/images/medium/'.$escort_product->city.'/'.$escort_product->code,0777);
                    }
                    if(!file_exists(REAL_PATH.'/uploads/images/full/'.$escort_product->city.'/'.$escort_product->code)){
                        mkdir(REAL_PATH.'/uploads/images/full/'.$escort_product->city.'/'.$escort_product->code);
                        chmod(REAL_PATH.'/uploads/images/full/'.$escort_product->city.'/'.$escort_product->code,0777);
                    }
                    
                    $img = array();
                    if (is_array($escort_imgs)) {
                        foreach ($escort_imgs as $key => $image_item) {
                            $unique_id = uniqid() . time() . uniqid();
                            if ($key == 0) {
                                $img[$unique_id]['primary'] = true;
                            }
                            $img[$unique_id]['filename'] = $escort_product->city.'/'.$escort_product->code.'/'.$image_item;
                            if(!is_dir(REAL_PATH.'/uploads/adv_images/'.$escort_product->city.'/'.$escort_product->id.'/small/'.$image_item)){
                                copy(REAL_PATH.'/uploads/adv_images/'.$escort_product->city.'/'.$escort_product->id.'/small/'.$image_item,
                                     REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item);
                                chmod(REAL_PATH.'/uploads/images/thumbnails/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item,0777);
                                
                                copy(REAL_PATH.'/uploads/adv_images/'.$escort_product->city.'/'.$escort_product->id.'/small/'.$image_item,
                                     REAL_PATH.'/uploads/images/small/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item);
                                chmod(REAL_PATH.'/uploads/images/small/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item,0777);

                                copy(REAL_PATH.'/uploads/adv_images/'.$escort_product->city.'/'.$escort_product->id.'/medium/'.$image_item,
                                     REAL_PATH.'/uploads/images/medium/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item);
                                chmod(REAL_PATH.'/uploads/images/medium/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item,0777);

                                copy(REAL_PATH.'/uploads/adv_images/'.$escort_product->city.'/'.$escort_product->id.'/large/'.$image_item,
                                     REAL_PATH.'/uploads/images/full/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item);
                                chmod(REAL_PATH.'/uploads/images/full/'.$escort_product->city.'/'.$escort_product->code.'/'.$image_item,0777);
                            }
                            $img[$unique_id]['alt'] = "";
                    ...
                    $slug = sha1($escort_product->id);
                    $route['slug'] = $slug;
                    $route_id = $this->Routes_model->save($route);

                    $save['id'] = "";
                    $save['sku'] = $escort_product->code;
                    $save['name'] = $escort_product->name;
                    $save['seo_title'] = $escort_product->name;
                    $save['meta'] = "";
                    $save['description'] = $escort_product->text;
                    $save['excerpt'] = "";
                    $save['price'] = "";
                    $save['saleprice'] = "";
                    $save['weight'] = "";
                    ...
                    $save['escort_phone'] = $escort_product->phone;
                    $save['escort_link'] = $escort_product->link;
                    $save['escort_city'] = $escort_product->city;
...
//                    $save['product_secret_code'] = $slug;
                    ...
                    if(strpos($escort_product->link,'FemaleEscorts')!=FALSE){ $category_products['category_id']='1'; };
                    if(strpos($escort_product->link,'TranssexualEscorts')!=FALSE){ $category_products['category_id']='2'; };
                    if(strpos($escort_product->link,'BodyRubs')!=FALSE){$category_products['category_id']='3';};
                    $category_products['product_id'] = $product_id;
                    //$category_products['category_id'] = $escort_product->category_id;
