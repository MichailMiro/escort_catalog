...
Class Product_model extends CI_Model
{
	...
	function product_autocomplete($name, $limit)
	{
        ...
        function get_last_create_products($limit = false, $offset = false)
	{
                $yesterday = date("Y-m-d", strtotime("-2 days"));
		//$this->db->where('DATE(create_time)',$yesterday);
                $this->db->where('enabled',1);
                $this->db->order_by('create_time', 'DESC');
                $result	= $this->db->limit($limit)->offset($offset)->get('products')->result();
                return $result;
	}
        function get_last_create_products_count()
	{
        ...
        function get_last_review_products()
	{
		//sort by alphabetically by default
                $this->db->where('enabled',1);
		$this->db->order_by('review_time', 'DESC');
		$result	= $this->db->get('products',4);
	...
	function get_products($category_id = false, $limit = false, $offset = false, $by=false, $sort=false)
	{
        ...
        function get_products_by_city($city = false, $limit = false, $offset = false, $by=false, $sort=false)
	{
		//if we are provided a category_id, then get products according to category
		if ($city)
		{
			$this->db->select('products.*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false)->from('products')->where(array('escort_city'=>$city));
	...
	function count_products($id)
	{
        ...
        function count_products_by_city($name)
	{
		return $this->db->select('product_id')->from('products')->where(array('escort_city'=>$name, 'enabled'=>1))->count_all_results();
	}
	...
	function get_cart_ready_product($id, $quantity=false)
	{
        ...
        function change_review_time($id){
            $this->db->where('id', $id);
            $this->db->set('review_time',date('Y-m-d H:i:s'));
            $this->db->update('products');
        }
        
        function add_your_сontent($data) 
	{
		$this->db->dbprefix = "";
        ...
        function get_all_сontents() 
	{
		$this->db->dbprefix = "";
                $res =  $this->db->get('user_contact');
                $this->db->dbprefix = "gc_";
                return $res->result();
	}
        
        function get_one_сontent($id) 
	{
            $this->db->dbprefix = "";
            $this->db->where('id', $id);
            $res = $this->db->get('user_contact');
        ...
        function delete_сontents() 
	{
        ...
        function change_added_status($id){
            $this->db->dbprefix = "";
            $this->db->where('id', $id);
            $this->db->set('added','1');
            $this->db->update('user_contact');
            $this->db->dbprefix = "gc_";
        }
        
        function get_city_list(){
            $this->db->select('escort_city');
            $this->db->group_by('escort_city');
            $res = $this->db->get('products');
            return $res->result();
        }
        
        function add_delete_request($data) 
	{
		$this->db->dbprefix = "";
                $this->db->insert('delete_request', $data);
                $this->db->dbprefix = "gc_";
		return $this->db->insert_id();
	}
        
        function get_active_delete_request(){
            //$this->db->select('escort_city');
            $this->db->where('status', '0');
            $this->db->dbprefix = "";
            $res = $this->db->get('delete_request');
            $this->db->dbprefix = "gc_";
            
            return $res->result();
        }
        
        function check_list($phone){
            $this->db->select('phone');
            $this->db->dbprefix = "";
            $this->db->from('baned_phone');
        ...
        function add_banned_phone($phone,$id) 
	{
                $this->db->dbprefix = "";
		$this->db->where('id', $id);
                $this->db->set('status',1);
                $this->db->update('delete_request');
                $this->db->dbprefix = "gc_";
            
                $this->db->dbprefix = "";
                $this->db->insert('baned_phone', $phone);
                $this->db->dbprefix = "gc_";
		return $this->db->insert_id();
	}
        ...
        function check_sku($sku)
	{
        ...
        function disabled_product_by_phone($phone){
            $this->db->where('escort_phone', $phone);
            $this->db->set('enabled',0);
            $this->db->update('products');
        }
        
        function add_watch_product($data) 
	{
                $this->db->dbprefix = "";
